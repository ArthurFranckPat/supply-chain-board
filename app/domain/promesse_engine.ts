...
export type PromesseMode = 'optimiste' | 'engageante'
 ...
 */
export function computePromesseDate(req: PromesseRequest, data: PromesseDataset): PromesseResult {
...
function dispoDate(
  ...
  const coverLeaves: PromesseNode[] = []
...
/**
 * Stock + flux mutables, clonés à l'entrée de `computePromesseDate`.
 ...
 */
function shiftDate(date: Date, days: number, mode: PromesseMode, closedDays?: Set<string>): Date {
...
function mkInfeasible(article: string, quantity: number, from: Date, detail: string): PromesseNode {
  return {
    article,
    quantity,
    availableDate: from,
...
function markCriticalChild(children: PromesseNode[]): void {
 ...
 */
function buildCriticalPath(root: PromesseNode): PromesseNode[] {
  const path = [root]
  let node = root
