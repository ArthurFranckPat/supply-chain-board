/**
 * Assemblage du payload `/api/v1/planning/ruptures/rows` — pipeline faisabilité +
 * réceptions, pivoté en lignes, formaté pour le Registre (badges verdict + dates FR).
 *
 * Extrait de `SchedulerController.ruptureRows` (issue #49). ruptures (coquille
...
const VERDICT_PRESET: Record<RuptureRow['verdict'], { label: string; cls: string }> = {
  couvert: {
    label: 'Couvert',
 ...
 * le tool agent `listerRuptures`). Renvoie les RuptureRow bruts + stats.
 *
 ...
 */
export async function loadRuptureRowsData(params: {
  start?: string
  days?: number
  force?: boolean
}): Promise<{
  ...
  try {
    const cached = await ruptCache().getOrSet({
      ...
      factory: async () => {
        ...
        // OfCommandePeg (Date) → RuptureOfPeg (ISO) pour le pivot pur.
        ...
        const built = buildRuptureRows(result, receptionsByArticle, articles, pegsIso, {
...
/**
 * GET /api/v1/planning/ruptures/rows — endpoint JSON (calcul lourd).
 * Charge le pipeline via `loadRuptureRowsData`, renvoie les lignes pré-formatées
 * + stats + erreur X3 (consommé en fetch par la page Solid `ruptures`).
 */
export async function loadRuptureRows(ctx: HttpContext) {
