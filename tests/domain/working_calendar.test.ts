...
test.group('working_calendar / factor', () => {
  ...
  test('fermeture poste : ne touche que le poste visé, dans la plage', ({ assert }) => {
    const closures: Closure[] = [
      { scope: 'wst', code: 'PP_830', from: '2026-08-10', to: '2026-08-21', factor: 0 },
    ]
  ...
  test('fermeture atelier : matche le STOLOC', ({ assert }) => {
    const cal = buildWorkingCalendar(new Set(), [
      { scope: 'stoloc', code: 'S9P', from: '2026-12-24', to: '2026-12-31', factor: 0 },
    ])
  ...
  test('fermeture globale demi-journée : 0.5 partout', ({ assert }) => {
    const cal = buildWorkingCalendar(new Set(), [
      { scope: 'global', code: '', from: '2026-07-13', to: '2026-07-13', factor: 0.5 },
    ])
  ...
  test("le plus restrictif l'emporte (férié > demi)", ({ assert }) => {
    const cal = buildWorkingCalendar(new Set(['2026-07-14']), [
      { scope: 'global', code: '', from: '2026-07-14', to: '2026-07-14', factor: 0.5 },
    ])
