 ...
 * Port React du Solid inertia/pages/programme.tsx (1255 l.).
 *
 ...
 * par poste, recherche multi-scope, drag&drop). Le programme n'ajoute que deux calques :
 ...
 * • plus de `components/programme/programme-toolbar` (supprimé) ni de menu
...
import BatchAffermirBar from '@r/components/board/batch-affermir-bar'
...
import { virtualOrdersFrom } from '@r/lib/scenario/types'
...
const MODE_TITLES: Record<ProgrammeMode, string> = {
  ...
  commandes: 'Mode Commandes — board par ligne de commande',
}
...
export default function Programme(props: ProgrammeProps) {
  ...
  const switchMode = useCallback(
    (newMode: ProgrammeMode) => {
      ...
      const wasOrderMode = mode === 'commandes'
      const willBeOrderMode = newMode === 'commandes'
  ...
  const isOrderMode = mode === 'commandes'
  ...
  const applyRange = useCallback(
    (r: DayPickerRange) => {
      ...
      if (r.from && r.to) {
        ...
        router.visit(route('programme.index'), {
          data: {
            start: toIso(r.from),
            days: String(days),
            ...(mode !== 'combined' && { mode: mode }),
    ...
    <>
      ...
      <ToolbarGroup>
        <ToolbarSegmented semantics="tabs" aria-label="Mode d'affichage">
          {(['ordonnancement', 'combined', 'commandes'] as const).map((m) => (
            <ToolbarSegment
              key={m}
              active={mode === m}
              title={MODE_TITLES[m]}
      ...
      {/* Scénario, triage et sélection ne sont pas des actions : ce sont des
          ...
          scénario, rail, BatchAffermirBar). Trois pills permanentes pour trois
  ...
  return (
    ...
    >
      ...
      <div className="flex h-full min-h-0 flex-col">
        ...
        {/* BatchAffermirBar (OF seulement) */}
        {mode !== 'commandes' && <BatchAffermirBar />}
      </div>
