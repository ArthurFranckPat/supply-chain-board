...
export interface RuptureReception {
...
export interface RuptureRow {
  ...
  reception: RuptureReception | null
  couverte: boolean
  /**
  ...
  joursMarge: number
  /**
   * Continuum de ponctualité (cf. bloc verdict dans buildRuptureRows) :
 ...
 * cumul insuffisant.
 *
 * PARTAGÉE par les deux vues : table Ruptures (buildRuptureRows ci-dessous) + ETA goulots
...
export function buildRuptureRows(
  ...
  for (const p of pending) {
    ...
    let verdict: RuptureRow['verdict']
    if (!reception) {
