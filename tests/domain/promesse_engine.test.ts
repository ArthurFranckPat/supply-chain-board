...
function mkDataset(
  ...
  extra: Partial<Pick<PromesseDataset, 'ofSupply' | 'supplierLatency' | 'closedDays'>> = {}
): PromesseDataset {
...
function promise(
  ...
  mode: PromesseMode = 'optimiste'
) {
  return computePromesseDate({ article, quantity, from: FROM, mode }, data)
}
