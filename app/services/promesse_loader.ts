/**
 * Loader CTP — dérive PromesseDataset des caches boardDataset (SWR partagé).
 *
...
/**
 * Construit le PromesseDataset pour un article, en réutilisant les caches
 * boardDataset. Définit le périmètre stock/réceptions/OF aux articles
 * atteignables depuis la BOM du produit demandé.
 */
export async function buildPromesseDataset(article: string): Promise<PromesseDataset> {
 ...
 * Chaque mode obtient son propre ledger (cloné dans computePromesseDate) →
 * les deux passes sont indépendantes.
 */
export async function loadPromesse(params: {
  article: string
  quantity: number
  from?: Date
}): Promise<PromiseResponse> {
  const from = params.from ?? new Date()
  const data = await buildPromesseDataset(params.article)

  const base = { article: params.article, quantity: params.quantity, from }
  const optimiste = computePromesseDate({ ...base, mode: 'optimiste' }, data)
  const engageante = computePromesseDate({ ...base, mode: 'engageante' }, data)
