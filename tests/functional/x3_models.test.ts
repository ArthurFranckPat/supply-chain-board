...
test.group('X3 Models', () => {
  ...
  test('X3Database raw query returns same data', async ({ assert }) => {
    ...
    const rows = await db.raw('SELECT SOHNUM_0, BPCNAM_0, ORDDAT_0 FROM SORDER WHERE ROWNUM <= 3')
  ...
  test('x3db from container (like controllers use) returns SORDER data', async ({ assert }) => {
    ...
    const rows = await db.from('SORDER').select('SOHNUM_0', 'BPCNAM_0').limit(2)
  ...
  test('X3StockRepository returns stock flows from ITMMVT', async ({ assert }) => {
    ...
    for (const f of flows.slice(0, 3)) {
      ...
      console.log(
        `[STOCK] article=${f.article}  qte=${f.quantity}  subType=${o.subType}  pmp=${o.pmp}`
      )
    }
    ...
    for (const f of flows.slice(0, 3)) {
      ...
      const fmtFR = (d: Date | null) =>
        d
          ...
          : 'N/A'
      console.log(
        `[REC] id=${o.id}  article=${f.article}  qte=${f.quantity}  dateFin=${fmtFR(f.date)}  fournisseur=${o.supplier}`
      )
    }
  })
  ...
  test('X3GammeRepository returns first operation per article', async ({ assert }) => {
    ...
    for (const op of ops.slice(0, 3)) {
      console.log(
        `[GAMME] article=${op.article}  poste=${op.workstation}  libelle=${op.workstationLabel}  cadence=${op.rate}`
      )
    }
  ...
  test('X3OfRepository resolves statutLabel from local_menus', async ({ assert }) => {
    ...
    for (const f of ofFlows) {
      ...
      console.log(
        `[OF] id=${o.id}  article=${f.article}  qte=${f.quantity}  statut=${o.status}  statutLabel=${o.statutLabel}  typeOf=${o.typeOf}  typeOfLabel=${o.typeOfLabel}  designation=${o.designation}`
      )
