...
export const FriseView: Component<{
  ...
  const bounds = createMemo(() => timeBounds(props.rows.map((c) => [c.debut, c.fin] as const)))
      ...
      <div class="overflow-hidden rounded-xl border border-rule bg-card">
        ...
        <For
          ...
          fallback={
            <div class="flex flex-col items-center gap-2 p-10 text-center text-muted-foreground">
              ...
              <span class="font-fraunces text-[14px] italic">
        ...
        >
          {(c) => {
            ...
            const width = () =>
              Math.max(pctOf(toMinutes(c.fin), bounds().start, dur()) - left(), 2.5)
            const taux = () => tauxOf(c)
              ...
              <div class="grid grid-cols-[210px_1fr] border-b border-rule-soft last:border-b-0 hover:bg-foreground/[0.03]">
                <div class="flex flex-col gap-0.5 border-r border-rule-soft px-4 py-2 min-w-0">
                  <div class="flex items-center gap-1.5">
                    ...
                    <Show when={c.source === 'navette'}>
                      <span class="font-mono text-[8px] font-bold tracking-wider text-brand uppercase">
                        {c.navetteNum}
                      </span>
                    </Show>
                  </div>
                  ...
                  <Show when={!c.anomalie && taux() > 1}>
                    <span
                      class="flex w-fit items-center gap-0.5 font-mono text-[8.5px] font-bold tracking-[0.05em] text-destructive uppercase"
                ...
                <div class="relative" style={{ height: `${ROW_H}px` }}>
                  ...
                  <Show when={nowPct() !== null}>
                    <div
                      class="pointer-events-none absolute inset-y-0 z-[5] w-px bg-brand"
                      style={{ left: `${nowPct()}%` }}
                    >
        ...
        <div class="grid grid-cols-[210px_1fr] border-t-2 border-rule bg-secondary">
          <div class="flex flex-col justify-center gap-0.5 border-r border-rule-soft px-4 py-3">
            <span class="font-mono text-[9px] font-bold tracking-[0.14em] text-secondary-foreground uppercase">
              Densité quai
            </span>
            <span class="font-mono text-[8px] font-medium text-muted-foreground italic">
                  ...
                  class={cx(
                    ...
                    chargeText(
                      chargeTier(
                        c().palTheo >= 0
                          ? c().tauxRemplissage
                          : c().nbPalettes / props.camionCapacitePalettes
                      )
                    )
                  )}
                >
                  {c().nbPalettes} pal.
                  <Show when={c().palTheo >= 0}> (≈ {c().palTheo.toFixed(1)} théo.)</Show>
                </b>
              </div>
              <Show when={c().palTheo >= 0}>
                <div class="flex justify-between gap-3.5 font-mono text-[10px] text-muted-foreground">
                  ...
                  <b class="font-bold text-foreground">
                    {Math.round(c().tauxRemplissage * 100)}% / {props.camionCapacitePalettes} pal.
                  </b>
                </div>
              </Show>
              <div class="flex justify-between gap-3.5 font-mono text-[10px] text-muted-foreground">
                <span>UC</span>
                <b class="text-foreground font-bold">{c().qteUc.toLocaleString('fr-FR')}</b>
              </div>
              <div class="flex justify-between gap-3.5 font-mono text-[10px] text-muted-foreground">
                <span>Contenants</span>
                <b class="text-foreground font-bold">{c().nbContenants}</b>
              </div>
              <div class="flex justify-between gap-3.5 font-mono text-[10px] text-muted-foreground">
                <span>Lignes</span>
                <b class="text-foreground font-bold">{c().nbLignes}</b>
              </div>
            </>
          )}
        </Show>
