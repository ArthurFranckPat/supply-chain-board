...
export function RapportsView({ embeddedReports }: RapportsViewProps) {
  ...
  const reports = embeddedReports ? Object.values(embeddedReports).filter(Boolean) : []
        ...
        return (
          <Card key={String(report.type)}>
            <CardHeader
              ...
              onClick={() => open({ title: String(report.type), description: String(report.path ?? ''), payload: report })}
            >
              <CardTitle>{String(name)}</CardTitle>
            </CardHeader>
            <CardContent>
              <pre className="text-[11px] bg-muted p-2 overflow-auto max-h-80 font-mono whitespace-pre-wrap">
