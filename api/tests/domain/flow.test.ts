...
test.group('Flow helpers', () => {
  ...
  test('netQuantity sums supply positively and demand negatively', ({ assert }) => {
    const flows: Flow[] = [
      ...
      makeFlow({ article: 'A', direction: 'demand', quantity: 30, origin: { type: 'order', id: 'C1', customer: 'X', orderType: 'MTO', nature: 'COMMANDE' } }),
  ...
  test('netQuantity filters by date', ({ assert }) => {
    ...
    const flows: Flow[] = [
      ...
      makeFlow({ article: 'A', direction: 'supply', quantity: 20, date: d2, origin: { type: 'reception', id: 'R2', supplier: 'S' } }),
    ]
