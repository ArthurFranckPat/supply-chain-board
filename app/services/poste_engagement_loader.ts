 ...
 *
 * Le dataset sous-jacent (fetch X3 + matching) N'EST PAS scopé à un poste — le
 * filtre `wst === poste` n'intervenait qu'à la fin. `loadEngagementDataset` calcule
 * donc TOUS les postes en un seul passage (1 cache partagé) ; `loadPosteEngagement`
 * (contrat JSON inchangé, consommé par le panneau `PosteEngagementSheet`) et
 * `loadAllPostesEngagement` (page `/sequenceur`) lisent ce même dataset.
 */
...
async function computeEngagementDataset(force: boolean): Promise<EngagementDataset> {
  ...
  // planifiés/suggérés pour allouer comme le board). RÉFÉRENTIEL, articles, overrides.
  ...
  const resolvePoste = (mo: ManufacturingOrder): string | null => {
    const ov = overrideMap.get(mo.numOf)
    return ov?.workstation ?? gammeMap.get(mo.article)?.workstation ?? null
  }

  // TOUS les OF fermes, groupés par poste résolu (override d'abord, comme le board).
  const fermes = ord.mos.filter((mo) => mo.status === 1 && resolvePoste(mo) !== null)
  const fermeNums = new Set(fermes.map((m) => m.numOf))
  const fermesByPoste = new Map<string, ManufacturingOrder[]>()
  for (const mo of fermes) {
    const poste = resolvePoste(mo)!
  ...
  // ensuite : commande→OF ⇒ OF→commandes, filtré à TOUS les fermes (tous postes).
  ...
  const buildRows = (mos: ManufacturingOrder[]): EngagementRow[] =>
    mos
      .map((mo) => {
        const ov = overrideMap.get(mo.numOf)
  ...
  // inconnu ou schéma horaire vide → la vue saute le comparatif.
  const weeklyCapacityOf = (poste: string): number | null => {
  ...
  // Univers des postes affichés : tout poste référencé par une gamme (routage
  // connu) ou par un override — surperset de tout poste pouvant avoir un OF
  // ferme résolu. Inclut les postes à 0 OF ferme (page /sequenceur, filtre).
  ...
  const x3Error = errors.length ? errors.join(' | ') : null

  const postes: PosteEngagement[] = [...posteCodes].map((code) => {
    const rows = buildRows(fermesByPoste.get(code) ?? [])
    return {
      poste: { code, label: labelOf(code) },
      count: rows.length,
      totalHours: Math.round(rows.reduce((s, r) => s + r.hours, 0) * 100) / 100,
      weeklyCapacityHours: weeklyCapacityOf(code),
      rows,
      x3Error,
    }
  })
...
async function loadEngagementDataset(force = false): Promise<EngagementDataset> {
  const cacheKey = `all:${isoDay(new Date())}`
  if (force) await engagementCache().delete({ key: cacheKey })
  return engagementCache().getOrSet({
    key: cacheKey,
    ttl: ENGAGEMENT_TTL,
    factory: () => computeEngagementDataset(force),
  })
}

const emptyPosteEngagement = (poste: string): PosteEngagement => ({
  poste: { code: poste, label: poste },
  count: 0,
  totalHours: 0,
...
export async function loadPosteEngagement(poste: string, force = false): Promise<PosteEngagement> {
  const dataset = await loadEngagementDataset(force)
  return dataset.postes.find((p) => p.poste.code === poste) ?? emptyPosteEngagement(poste)
}

/** Page /sequenceur — tous les postes, pour la vue transverse avec filtres. */
export async function loadAllPostesEngagement(force = false): Promise<EngagementDataset> {
  return loadEngagementDataset(force)
}
