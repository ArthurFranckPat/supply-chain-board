...
import { diffCbnDrivers, type DriverDiffEntry } from '#app/domain/snapshot_driver_diff'
import {
  entreesPourArticle,
  parseCle,
  plusProcheDe,
...
export class DemandSnapshotService {
   ...
   * confondues) — `protected` pour rester COMPTABLE en test (une sous-classe
   * de test peut l'intercepter pour vérifier le nombre RÉEL de lectures
   * faites, cf. `demand_snapshot_service.test.ts`).
   *
   * Wrap explicite en promesse native (`.then`) : un query builder Lucid est
   * un thenable réexécuté à chaque `.then()` — chaque appelant qui partage
   * cette promesse attend la même requête déjà en vol.
   */
    ...
    const cached = await cacheNs('appro').getOrSetForever({
      ...
      factory: async ({ skip }) => {
        ...
        const [avantRows, apresRows] = await Promise.all([
          this.lireJourneeBrute(avantDay),
          this.lireJourneeBrute(apresDay),
        ])
