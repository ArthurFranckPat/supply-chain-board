/** Champs de `ChargeLine` dont la recherche client a besoin. */
export type ChargeSearchable = {
  code: string
  name: string
...
export function loadSearchRank(
  line: Pick<ChargeSearchable, 'code' | 'name' | 'articles'>,
  q: string
): number | null {
  if (!q) return 0
 ...
 */
export function filterChargeLines<T extends ChargeSearchable>(
  lines: readonly T[],
  query: string,
