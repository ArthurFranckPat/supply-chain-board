...
def compute_feasibility_ratio(results: dict[str, FeasibilityResult]) -> RatioStats:
    """Compute feasible/total ratio for feasibility result maps."""
    total = len(results)
    feasible = sum(1 for result in results.values() if result.feasible)
    pct = (feasible / total * 100) if total > 0 else 0.0
    return RatioStats(feasible=feasible, total=total, pct=pct)


def compute_allocation_ratio(results: dict[str, AllocationResult]) -> RatioStats:
    """Compute feasible/total ratio for allocation result maps."""
    total = len(results)
    feasible = sum(1 for result in results.values() if result.status == AllocationStatus.FEASIBLE)
    pct = (feasible / total * 100) if total > 0 else 0.0
    return RatioStats(feasible=feasible, total=total, pct=pct)
...
def format_summary(
    ...
    immediate_stats = compute_feasibility_ratio(immediate_results)
    console.print("[bold]Verification immediate (stock actuel)[/bold]")
    console.print(
        f"   ✅ Faisables : {immediate_stats.feasible}/{immediate_stats.total} "
        f"({immediate_stats.pct:.1f}%)"
    )
    console.print(
        f"   ❌ Non faisables : "
        f"{immediate_stats.total - immediate_stats.feasible}/{immediate_stats.total}"
    )

    projected_stats = compute_feasibility_ratio(projected_results)
    console.print("\n[bold]Verification projetee (stock + receptions)[/bold]")
    console.print(
        f"   ✅ Faisables : {projected_stats.feasible}/{projected_stats.total} "
        f"({projected_stats.pct:.1f}%)"
    )
    console.print(
        f"   ❌ Non faisables : "
        f"{projected_stats.total - projected_stats.feasible}/{projected_stats.total}"
    )

    if allocation_results:
        allocation_stats = compute_allocation_ratio(allocation_results)
        console.print("\n[bold]Allocation avec gestion de la concurrence[/bold]")
        console.print(
            f"   ✅ Alloues : {allocation_stats.feasible}/{allocation_stats.total} "
            f"({allocation_stats.pct:.1f}%)"
        )
        console.print(
            f"   ❌ Non alloues : "
            f"{allocation_stats.total - allocation_stats.feasible}/{allocation_stats.total}"
        )
