...
function TreeNode({ node, depth }: TreeNodeProps) {
  return (
    <li className={depth > 0 ? 'ml-5 border-l border-rule-soft pl-3' : ''}>
      ...
      >
        {node.children.length > 0 && <span className="text-muted-foreground/70 text-xs">▸</span>}
        <DynamicIcon
        ...
        {node.onCriticalPath && (
          <span className="text-[10px] font-bold text-suggere">Critique</span>
        )}
      </div>
...
export default function Promesse() {
  ...
  return (
    <AppLayout
      ...
      theme="airbnb"
    >
      <div className="mx-auto max-w-3xl py-1 md:py-5">
        {/* Formulaire */}
        <form
          onSubmit={submit}
          className="rounded-lg border border-rule-soft bg-card p-4 shadow-sm md:p-5"
        >
          <h2 className="mb-4 text-sm font-bold text-foreground/80">
            <Headset size={18} className="align-middle text-primary" /> Simulateur de promesse
            client
          </h2>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-[1fr_120px_160px_auto]">
            ...
            <div className="col-span-2 flex items-end md:col-span-1">
              <button
                type="submit"
                disabled={loading}
                className="w-full rounded-lg bg-primary px-5 py-2.5 text-sm md:w-auto md:py-2 font-semibold text-white transition-colors hover:bg-primary/90 disabled:opacity-50"
              >
                {!loading ? (
                  <>
                    <Zap size={18} className="align-middle" /> Promettre
                  </>
                ) : (
                  'Calcul…'
                )}
