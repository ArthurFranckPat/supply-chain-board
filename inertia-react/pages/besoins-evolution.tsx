 ...
 * • la barre suit le standard §18 : portée (`ToolbarDateWindow`, seuls les
 *   jours photographiés sont cliquables) / FilterMenu (source + nature) /
 *   recherche / actualiser — plus de `vision/toolbar` ;
 * • chrome de table (`TableHead` / `TableRow` / `TableCell`, thead collant,
 *   `text-right!` sur l'Écart), bandeaux de validité et empty — calqués sur
 *   /charge, /cockpit, /programme. La gouttière Primer n'est pas touchée.
 *
 * Une grille de 8 cartes posée au milieu de la page a précédé les filtres :
 * c'était un filtre déguisé en tuiles de KPI, la plupart affichant « — ».
 * Ne pas la réintroduire — un filtre n'est pas une donnée.
 */
...
/** Gravité des chips Nature — mêmes teintes que la gouttière, bornées aux
 *  quatre points de `ToolbarFilterChip`. Sous thème cursor, `--planifie` et
 *  `--ferme` coïncident : « Date » retombe sur `ok`, comme « Apparue ». */
const NATURE_CHIP_TONE: Record<DriverNature, 'ok' | 'critical' | 'warning' | 'neutral'> = {
  apparue: 'ok',
  disparue: 'critical',
  quantite: 'warning',
  date: 'ok',
  renumerotation: 'neutral',
}
...
/** ISO YYYY-MM-DD → Date locale (évite le recul UTC de `new Date(iso)`). */
  ...
  return (
    <div className="flex min-w-0 items-baseline gap-2">
      ...
      <span className="truncate text-xs leading-tight text-muted-foreground">
        {a.designation ?? 'sans désignation'}
...
export default function BesoinsEvolution() {
  ...
  const parNature = frise?.parNature ?? {}
  ...
  const joursPhoto = useMemo(() => new Set(photos.map((p) => p.date)), [photos])
  const windowValue: DayPickerRange | undefined =
    avant && apres ? { from: parseIsoLocal(avant), to: parseIsoLocal(apres) } : undefined
  const commitWindow = (range: DayPickerRange) => {
    if (!range.from || !range.to) return
    const a = isoLocalDay(range.from)
    const b = isoLocalDay(range.to)
    // Une photo ne se compare pas à elle-même — le calendrier n'a qu'un jour
    // par capture (`date` est la clé).
    if (a === b) return
    if (a < b) {
      setAvant(a)
      setApres(b)
  ...
  const toggleNature = (n: DriverNature) => {
  ...
  /**
   * Chip source. Jamais désactivée : dans la frise, une source écartée sur un
   * pas peut avoir bougé sur les autres — la griser masquerait des mouvements
   * réels. Le bandeau de périmètre dit où elle manque, le compteur vient du
   * serveur.
   */
  const chipSource = (src: DriverSource) => (
    <ToolbarFilterChip
      key={src}
  ...
  const activeFilterCount = (sourceFilter ? 1 : 0) + (estNatureParDefaut ? 0 : 1)
  const showFrise =
    !photosLoading && photos.length >= 2 && !hasMessage && !friseError && !friseLoading && frise

  /* ── Barre d'outils (standard §18) ───────────────────────────────────────
     Zone 01 : `ToolbarDateWindow` — seuls les jours qui ont une photo CBN
     sont cliquables. Zone 02 : un FilterMenu (source + nature). Zone 03 :
     recherche. Zone 04 : actualiser. Pas de conteneur `<Toolbar>` : la prop
     d'AppLayout en est déjà un. */
  const toolbar = (
    <>
      <ToolbarGroup>
        <ToolbarDateWindow
          ...
          disabled={photosLoading || photos.length < 2}
          disabledDays={(day) => !joursPhoto.has(isoLocalDay(day))}
          title="Photos CBN qui bornent la frise — seuls les jours photographiés sont choisissables"
        />

        <ToolbarFilterMenu activeCount={activeFilterCount} width={340} align="start">
          <ToolbarFilterSection>Source</ToolbarFilterSection>
          ...
          <div className="mt-1.5">
            <ToolbarFilterSection>Réalité</ToolbarFilterSection>
            <ToolbarSegmented semantics="toggles" flat className="w-full flex-wrap">
              {SOURCES_REALITE.map(chipSource)}
            </ToolbarSegmented>
          </div>
          <div className="mt-1.5">
            <ToolbarFilterSection>Propositions CBN</ToolbarFilterSection>
            <ToolbarSegmented semantics="toggles" flat className="w-full flex-wrap">
              {SOURCES_PROPOSITIONS.map(chipSource)}
            </ToolbarSegmented>
          </div>

          <Separator className="my-2" />
          <ToolbarFilterSection>Nature de mouvement</ToolbarFilterSection>
          <ToolbarSegmented semantics="toggles" flat className="w-full flex-wrap">
            {NATURES.map((n) => (
              <ToolbarFilterChip
                key={n}
      ...
      <ToolbarRefresh
        ...
        onClick={() => (avant && apres ? void reloadFrise() : void reloadPhotos())}
      />
    </>
  )
  ...
  return (
    ...
    >
      <div className="flex h-full min-h-0 flex-col">
        <div className="hidden flex-none items-baseline justify-between border-b border-border px-5 pb-3 pt-1 print:flex">
          <span className="text-[20px] font-semibold tracking-tight text-foreground">
            Évolution des besoins{' '}
            <span className="ml-3 font-mono text-sm font-normal tabular-nums text-muted-foreground">
              {rangeLabel}
            </span>
          </span>
        ...
        {photosError && (
          <div className="flex flex-none items-center gap-2 border-b border-destructive/30 bg-destructive/10 px-5 py-2 text-xs text-foreground">
            ...
            <span className="truncate font-mono">{photosError}</span>
          </div>
        )}
        {friseError && (
          <div className="flex flex-none items-center gap-2 border-b border-destructive/30 bg-destructive/10 px-5 py-2 text-xs text-foreground">
            <TriangleAlert size={16} strokeWidth={1.75} className="shrink-0 text-destructive" />
            <span className="font-semibold">Erreur chargement :</span>
            <span className="truncate font-mono">{friseError}</span>
          </div>
        )}
        ...
        {photosLoading ? (
          <LoadingState
            ...
            orbState="searching"
            title="Photos…"
            description="inventaire des captures CBN"
          />
        ) : photos.length < 2 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
            <p className="text-sm font-medium text-foreground">L&apos;historique commence</p>
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-2 p-10 text-center">
            {/* Couvre les trois cas : pas assez de photos, photos illisibles,
                plage trop large pour la frise. */}
            <p className="text-sm font-medium text-foreground">Frise indisponible</p>
            <p className="max-w-sm text-xs leading-relaxed text-muted-foreground">
              {frise?.message}
            </p>
            {avant && apres && (
          ...
          <>
            {/* Trous de la série + pas illisibles. Avertissement sur la VALIDITÉ
                de la frise : il apparaît/disparaît au changement de plage sans
                rechargement — annoncé aux lecteurs d'écran. Jamais silencieux :
                un trou est un pas dont les mouvements ne sont pas localisables
                au jour près. */}
            {(trous.length > 0 || pasIllisibles.length > 0) && (
              <div
                role="status"
                aria-live="polite"
                className="flex flex-none items-start gap-2 border-b border-suggere/30 bg-suggere/10 px-5 py-2 text-xs leading-relaxed text-foreground"
              >
                <TriangleAlert
                  aria-hidden
                  size={16}
                  strokeWidth={1.75}
                  className="mt-0.5 shrink-0 text-suggere"
                />
                  ...
                  {trous.map((t) => {
                    ...
                    return (
                      <span key={t.entre} className="block">
                        ...
                        <span className="font-mono tabular-nums">{fmtJour(t.entre)}</span> et le{' '}
                        <span className="font-mono tabular-nums">{fmtJour(t.et)}</span> : les
                        mouvements de ce pas ne sont pas localisés au jour près.
                      </span>
                    )
                  })}
                  {pasIllisibles.map((p) => (
                    <span key={`${p.avant}-${p.apres}`} className="block">
                      Pas du <span className="font-mono tabular-nums">{fmtJour(p.avant)}</span> au{' '}
            ...
            {/* Périmètre restreint (#145), agrégé sur toute la plage : une
                source écartée sur un pas reste présente sur les autres — le
                bandeau nomme les pas où elle manque. */}
              ...
              >
                ...
                <span>
                  ...
                  {groupesEcartements.map((g) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="font-semibold">{g.sources.join(', ')}</span> — {phrase} de
                        la photo « {g.manqueDans === 'avant' ? 'avant' : 'après'} », sur{' '}
                        {g.pas.length} pas :{' '}
                  ...
                  <span className="block">
                    Une source en échec n&apos;a pas été capturée ; une source sans journal est
                    écartée par prudence ; une source réellement vide, elle, reste comparée et sa
                    disparition est affichée.
                  </span>
                </span>
              </div>
            )}
            ...
            {total > servis && (
              <div className="flex flex-none items-center gap-2 border-b border-suggere/30 bg-suggere/10 px-5 py-2 text-xs text-foreground">
                ...
                <span className="font-mono tabular-nums">
                  {servis} affichés sur {fmtQte.format(total)}
                </span>
                <span className="text-muted-foreground">— affinez par source ou nature.</span>
              </div>
            )}

            <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
              <div className="flex flex-none items-center gap-2 border-b border-border px-5 py-1.5">
                <span className="font-mono text-[10px] tabular-nums text-muted-foreground">
                  {mouvementsAffiches} mouvement{mouvementsAffiches > 1 ? 's' : ''} · articles par
                  nombre de mouvements décroissant
                </span>
              ...
              {total === 0 ? (
                ...
                <div className="min-h-0 flex-1 overflow-auto px-5 py-4">
                  <div className="overflow-auto rounded-lg border border-border bg-card shadow-float">
                    <table className="w-full border-collapse text-left text-sm">
                      <thead>
                        <TableHeadRow className="sticky top-0 z-10">
                          <TableHead className="bg-card">Jour</TableHead>
                          <TableHead className="bg-card">Pièce</TableHead>
                          <TableHead className="bg-card">Source</TableHead>
                          <TableHead className="bg-card">Nature</TableHead>
                          <TableHead className="bg-card">Échéance</TableHead>
                          <TableHead className="bg-card">Avant → Après</TableHead>
                          <TableHead align="right" className="bg-card text-right!">
                            Écart
                          </TableHead>
                        </TableHeadRow>
                      </thead>
                        ...
                        {filteredArticles.map((a) => (
                          <Fragment key={a.article}>
                            {/* L'en-tête du groupe rappelle l'article sur
                                ...
                                lignes sans perdre le contexte. Fond sur la
                                cellule : sous cursor, le `tr` est forcé
                                transparent. */}
                            <TableRow
                              clickable
                            ...
                            >
                              <TableCell
                                colSpan={7}
                                className="bg-muted/40 py-1.5 group-hover/row:bg-muted/60"
                              >
