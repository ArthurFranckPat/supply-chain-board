...
export function HeuresCapaciteChart({ data }: { data: HeuresMoisView[] }) {
  const geom = useMemo(() => {
    ...
    const bars = data.map((d, i) => {
      ...
      return { mois: d.mois, cx, x: cx - barW / 2, y: y(d.capacite), h: PAD_T + plotH - y(d.capacite), w: barW }
    })
    ...
    return { bars, pointees, converties, pathPointees: pathOf(pointees), pathConverties: pathOf(converties), max, baseline: PAD_T + plotH }
  }, [data])
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="block h-[170px] w-full" role="img">
      ...
      <path d={geom.pathConverties} fill="none" stroke={FERME} strokeWidth="2" strokeLinecap="round" />
      {geom.converties.map((p, i) => (
        <circle key={i} cx={p.x} cy={p.y} r="3" fill={FERME} />
      ))}

      {/* Heures réellement pointées. */}
      <path d={geom.pathPointees} fill="none" stroke={BRAND} strokeWidth="2" strokeLinecap="round" />
      ...
      {geom.bars.map((b, i) => (
        <text key={b.mois} x={b.cx} y={H - 8} textAnchor="middle" fontSize="9" fill={FG} fillOpacity="0.55">
          {moisLabel(b.mois, i)}
        </text>
      ))}
    </svg>
