 ...
 *
 * Extrait de `ChargeController.index` (issue #49) : seul offender du controller (249 l.
...
/**
 * Shapes émis vers la page Inertia. Miroir côté client : inertia-react/lib/charge/types.ts
 * (même convention que SuiviController ↔ inertia-react/lib/suivi/types.ts).
 */
interface ChargePeriod {
  f: number
  p: number
  s: number
...
const emptyPeriod = (): ChargePeriod => ({ f: 0, p: 0, s: 0, fi: 0, si: 0 })
const round = (p: ChargePeriod): ChargePeriod => ({
  f: Math.round(p.f),
  p: Math.round(p.p),
  s: Math.round(p.s),
  fi: Math.round(p.fi),
...
export async function loadChargePayloadData(params: { start?: string; force?: boolean }) {
  ...
  return chargeCache().getOrSet({
    ...
    factory: async () => {
      ...
      const buildLines = (records: AggRecord[]): ChargeLine[] => {
          ...
          return ops
            ...
            .map((gamme) => {
              ...
              return {
                ...
                resteHours: hours,
                field: ofSegment(mo.status) as keyof ChargePeriod,
      ...
      const cmdLines = buildLines(
        ...
        chargeNeeds.map((n): AggRecord => ({
          ...
          field: chargeSegment(n.depth, n.nature) as keyof ChargePeriod,
          article: n.article,
        }))
      )
