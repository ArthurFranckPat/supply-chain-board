 ...
 * `orders_replica` (cf. la migration de cette table) — un `numOf` hors de ce
 * périmètre (ex. `controle_prod_loader`, MFGITM ouverts absents du live) ne
 * ressortira JAMAIS ici, silencieusement. Ce repository ne le sait pas et ne peut
 * pas le détecter : à l'appelant de vérifier que ses `numOfs` restent dans le
 * périmètre couvert avant de basculer sur cette voie (cf. `board_dataset.getOperations`).
 *
