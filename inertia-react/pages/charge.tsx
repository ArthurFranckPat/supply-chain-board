...
import type { ChargePageProps, ChargeLine, ChargeQtyMode, ChargeView } from '@r/lib/charge/types'
import { type Gran, maskPeriod, satRate, segKeys, segOptions, total } from '@r/lib/charge/chart-math'
import { filterChargeLines } from '@r/lib/charge/search'
import { HatchDefs } from '@r/components/charge/hatch-defs'
import { MiniCard } from '@r/components/charge/mini-card'
import { DetailChart } from '@r/components/charge/detail-chart'
import { ChargePeriodSheet } from '@r/components/charge/charge-period-sheet'
 ...
 * serveur (ChargeController) ; ici, pure présentation réactive.
 *
 ...
 */
const QTY_MODES: { id: ChargeQtyMode; label: string; long: string; hint: string }[] = [
  {
    id: 'brut',
    label: 'Brut',
    long: 'Brut',
...
export default function Load(props: ChargePageProps) {
  const [view, setView] = useState<ChargeView>('of')
  ...
  const [qtyMode, setQtyMode] = useState<ChargeQtyMode>('reste')
  const viewNet = useCallback(
    (l: ChargeLine): ChargeLine =>
      qtyMode === 'net'
  ...
  const filteredLines = useMemo(
    () => filterChargeLines(lines, query, atelierFilter),
    ...
    <>
      ...
      <ToolbarGroup>
        ...
        {/* Brut / Net / Reste n'est PAS un filtre : aucun poste, aucune période
            ...
            coïncident (charge_payload_loader.ts) — le contrôle n'y ferait rien. */}
        {view === 'commande' && (
