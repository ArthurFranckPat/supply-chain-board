...
- Creer un service dedie (ex: `EolResidualsService`) dans `apps/planning-engine/planning_engine/feasibility/eol_residuals.py`.
...
- Ajouter endpoint POST dans `apps/planning-engine/planning_engine/api/server.py`, ex: `/api/v1/eol-residuals/analyze`.
- Ajouter facade dans `apps/planning-engine/planning_engine/app/gui_service.py` (verif data chargee, appel service, serialisation standard).
