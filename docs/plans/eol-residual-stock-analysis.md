...
- Creer un service dedie (ex: `EolResidualsService`) dans `apps/ordo-core/src/checkers/eol_residuals.py`.
...
3. Exposition API Ordo-core
- Ajouter endpoint POST dans `apps/ordo-core/src/api/server.py`, ex: `/api/v1/eol-residuals/analyze`.
- Ajouter facade dans `apps/ordo-core/src/app/gui_service.py` (verif data chargee, appel service, serialisation standard).
