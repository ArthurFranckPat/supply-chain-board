...
import type { ChargePeriod, ChargeView } from '@r/lib/charge/types'
...
export const total = (p: ChargePeriod) => p.f + p.p + p.s + p.fi + p.si
 ...
 */
export interface ChargeSegOption {
  /** Identifiant du bouton (stable par vue). */
  id: string
  label: string
  /** Couleur de pastille de légende. */
  color: string
  /** Champs de `ChargePeriod` conservés quand l'option est active. */
  keys: (keyof ChargePeriod)[]
}

export const OF_SEG_OPTIONS: ChargeSegOption[] = [
  { id: 'f', label: 'Ferme', color: FERME, keys: ['f'] },
  { id: 'p', label: 'Planifié', color: PLANIFIE, keys: ['p'] },
  { id: 's', label: 'Suggéré', color: SUGGERE, keys: ['s'] },
]

export const CMD_SEG_OPTIONS: ChargeSegOption[] = [
  { id: 'commande', label: 'Commande', color: FERME, keys: ['f', 'fi'] },
  { id: 'prevision', label: 'Prévision', color: SUGGERE, keys: ['s', 'si'] },
]

export const segOptions = (view: ChargeView): ChargeSegOption[] =>
  view === 'of' ? OF_SEG_OPTIONS : CMD_SEG_OPTIONS

/** Champs de `ChargePeriod` retenus par un jeu d'options actives. */
export const segKeys = (view: ChargeView, active: ReadonlySet<string>): Set<keyof ChargePeriod> => {
  const keys = new Set<keyof ChargePeriod>()
...
export const maskPeriod = (p: ChargePeriod, on: ReadonlySet<keyof ChargePeriod>): ChargePeriod => ({
  f: on.has('f') ? p.f : 0,
  p: on.has('p') ? p.p : 0,
  s: on.has('s') ? p.s : 0,
  fi: on.has('fi') ? p.fi : 0,
...
export const segLabel = (view: ChargeView, key: keyof ChargePeriod): string => {
...
export const segsOf = (d: ChargePeriod): [keyof ChargePeriod, number, string][] => [
  ['s', d.s, SUGGERE],
...
export function segmentDeCle(k: keyof ChargePeriod, view: ChargeView): SegmentCharge {
  switch (k) {
    case 'fi':
 ...
 */
export function segmentsDeVue(view: ChargeView, actifs?: ReadonlySet<string>): SegmentCharge[] {
