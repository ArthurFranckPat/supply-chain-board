...
import { filterLoadLines, type LoadSearchable } from '../../inertia-react/lib/load/search.ts'

const line = (code: string, name: string, articles: string[], atelier = 'S9P'): LoadSearchable => ({
  code,
  name,
  articles,
  atelier,
...
test.group('filterLoadLines — recherche /charge', () => {
  test('« 830 » : PP_830 d’abord (code), puis les articles par code poste', ({ assert }) => {
    assert.deepEqual(
      filterLoadLines(EXAMPLE, '830').map((l) => l.code),
  ...
  test('sans requête : ordre d’entrée conservé', ({ assert }) => {
    assert.deepEqual(
      filterLoadLines(EXAMPLE, '').map((l) => l.code),
  ...
  test('code exact avant code qui contient', ({ assert }) => {
    ...
    assert.deepEqual(
      filterLoadLines(lines, 'PP_830').map((l) => l.code),
      ['PP_830', 'PP_830S']
    )
  })
  ...
  test('libellé avant article', ({ assert }) => {
    ...
    assert.deepEqual(
      filterLoadLines(lines, 'easy').map((l) => l.code),
      ['PP_830', 'PP_091']
    )
  })
  ...
  test('filtre atelier toujours appliqué', ({ assert }) => {
    ...
    assert.deepEqual(
      filterLoadLines(lines, '830', new Set(['S9P'])).map((l) => l.code),
      ['PP_830']
    )
  })
})
