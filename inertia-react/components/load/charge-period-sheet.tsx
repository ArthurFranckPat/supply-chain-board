...
/** Clé d'une ligne de commande — celle des overrides de date. */
const cleLigne = (numCommande: string | null, ligne: string | null): string =>
  `${numCommande ?? ''}#${ligne ?? ''}`
...
/**
 * Où en est un OF — statut de l'ordre X3 et, s'il est ferme, son état de suivi
 * MFGHEAD. Porté à l'identique par les OF de la vue OF et par les OF alloués de
 * la vue commande (`ChargeOfEtat` côté serveur).
 */
interface OfEtat {
  /** 1 ferme, 2 planifié, 3 suggéré. */
  statut: number
  statutLabel: string | null
  /** État de suivi MFGHEAD (En attente, Édité, En cours…) — OF ferme seulement. */
  suiviLabel: string | null
  /** Ferme ET dossier édité ou en cours atelier. */
  lance: boolean
}
...
interface DetailOfRow extends OfEtat {
  numOf: string
  article: string
 ...
 */
interface DetailRowOf extends OfEtat {
  numOf: string
...
export interface ChargePeriodSheetProps {
  ...
  /**
   * Appelé après chaque retour d'une ligne re-datée à sa date X3.
   *
...
export function ChargePeriodSheet(props: ChargePeriodSheetProps) {
  ...
  // Retour d'une ligne re-datée à sa date X3 (override local posé ailleurs,
  // /programme planification notamment).
  ...
  // En vue OF, la colonne « Statut » porte l'état de l'ordre (ferme / planifié
  // / suggéré, et lancé ou non) ; en vue commande, le même indicateur vit sur
  // chaque OF alloué de la colonne « OF ».
  const cols = unitPieces
    ? view === 'of'
      ? '9rem 1.4fr 8.5rem 9.5rem 8.5rem 1.1fr 7rem'
      : '9rem 1.3fr 1.4fr 9rem 1fr 1.5fr 7rem'
    : view === 'of'
      ? '9rem 1.4fr 8.5rem 9.5rem 8.5rem 1.1fr 7rem 7rem'
      : '9rem 1.3fr 1.4fr 9rem 1fr 1.5fr 9rem 7rem'

  const unitHead = unitPieces ? 'Pièces' : 'Heures'
  const heads = unitPieces
    ? view === 'of'
      ? ['Article', 'Désignation', 'Ordre', 'Statut', 'Commande', 'Client', unitHead]
      : ['Article', 'Désignation', 'Via', 'Commande', 'Client', 'OF', unitHead]
    : view === 'of'
      ? ['Article', 'Désignation', 'Ordre', 'Statut', 'Commande', 'Client', 'Qté', unitHead]
      : ['Article', 'Désignation', 'Via', 'Commande', 'Client', 'OF', 'Qté', unitHead]

  // Index de la colonne d'unité : c'est au bout d'ELLE que le total de période se lit.
  const unitIdx = heads.length - 1
        ...
        <TooltipProvider delay={150} closeDelay={50}>
          {loading ? (
            ...
            <>
              ...
              {actionError && (
                <div className="flex flex-none items-center gap-2 border-b border-destructive/30 px-5 py-1.5 text-[12px] text-destructive">
                  ...
                  <div className="grid items-center" style={{ gridTemplateColumns: cols }}>
                    ...
                    {heads.map((h, i) => (
                      <div
                        ...
                        title={
                          h === 'OF'
                            ? 'OF alloués à cette ligne par le moteur de matching commande→OF (comme /suivi) — quantité allouée et état de l’OF (ferme, planifié, suggéré ; lancé ou non)'
                            : h === 'Statut'
                              ? 'Statut de l’OF dans X3 et, pour un OF ferme, son état de suivi : lancé = dossier édité ou en cours atelier'
                              : h === 'Commande' && view === 'of'
                      ...
                      {heads.map((_, i) =>
                        i === 0 ? (
                          ...
                          <div
                            key={`ft-${i}`}
                            className={cn(
                              'pr-5 text-right font-mono text-[13px] font-bold tabular-nums text-foreground'
                            )}
                          >
                            {fmt(totalValue)}
                          </div>
  ...
  return (
    <>
      ...
      <div className={cn(CELL, 'truncate font-mono text-[10px] text-secondary-foreground')}>
        ...
        {/* Date locale substituée à celle de X3 (override posé ailleurs,
            /programme notamment) : le retour à la date X3 vit ici. */}
        {redatee && r.numCommande && r.ligne && (
          <button
            type="button"
            disabled={anyBusy}
            onClick={() => onRetablir(r.numCommande!, r.ligne!)}
...
/** Libellé complet de l'état d'un OF — infobulles. */
function etatTitle(e: OfEtat): string {
  const statut = e.statutLabel ?? STATUT_OF[e.statut]?.label ?? '—'
  if (e.statut !== 1) return statut
  if (!e.suiviLabel) return `${statut} · état de suivi inconnu`
  return `${statut} · ${e.lance ? 'lancé' : 'non lancé'} (${e.suiviLabel})`
}

const STATUT_OF: Record<number, { label: string; court: string; color: string }> = {
  1: { label: 'Ferme', court: 'F', color: 'var(--color-ferme)' },
  2: { label: 'Planifié', court: 'P', color: 'var(--color-planifie)' },
  3: { label: 'Suggéré', court: 'S', color: 'var(--color-suggere)' },
}

/**
 * État d'un OF : statut X3 (ferme / planifié / suggéré) et, pour un ferme, s'il
 * est lancé — dossier édité ou en cours atelier (MFGHEAD.MFGTRKFLG 3 ou 4).
 * Même composant dans les deux vues du détail ; `compact` pour la colonne OF de
 * la vue commande, où plusieurs OF se suivent sur une ligne.
 */
function OfEtatBadge({ etat, compact = false }: { etat: OfEtat; compact?: boolean }) {
  const s = STATUT_OF[etat.statut]
  if (!s) return null
  const color = s.color
  const suite =
    etat.statut !== 1
      ...
      : etat.lance
        ...
        : etat.suiviLabel
          ? compact
            ? 'non lancé'
            : etat.suiviLabel.toLowerCase()
          : null
  return (
    <span
      className={cn(
        'inline-flex items-baseline gap-1 rounded-sm px-1 py-px font-mono text-[9px] font-bold uppercase tracking-wider',
        compact && 'ml-1'
      )}
      style={{ color, background: `color-mix(in srgb, ${color} 14%, transparent)` }}
      title={etatTitle(etat)}
    >
      {compact ? s.court : s.label}
      {suite && (
        <span
          className={cn('normal-case tracking-normal', !etat.lance && 'font-medium opacity-75')}
        >
          · {suite}
        </span>
      )}
        ...
        return (
          <CommandeTooltip key={`${c.numCommande}-${c.ligne ?? ''}-${i}`} data={c}>
            <span className="mr-1.5 inline-flex cursor-help items-baseline gap-1">
              ...
              <span
                className={cn(pegue ? 'font-bold' : 'font-semibold text-secondary-foreground')}
                style={pegue ? { color: 'var(--color-ferme)' } : undefined}
              >
                {c.numCommande}
...
function OfClientsCell({ commandes }: { commandes: DetailOfCommande[] }) {
  ...
  const clients = [
    ...new Set(commandes.map((c) => c.client || (c.type === 'forecast' ? 'sans client' : '—'))),
  ]
