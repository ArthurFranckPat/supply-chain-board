 ...
 *
 * Shell Inertia instantané (SchedulerController.ruptures) ; les lignes (calcul
 * lourd : faisabilité + réceptions) chargées en différé par fetch JSON (ruptureRows).
...
import { RuptureRegistre, RuptureComposants } from '@r/components/ruptures'
import {
  ToolbarSegmented,
  ToolbarSegment,
  ToolbarSearch,
...
import { VERDICT_TONE } from '@r/lib/ruptures/rupture-math'
import type { RuptureRowsResponse, RuptureVerdictKey } from '@r/lib/ruptures/types'
...
/** Bornes serveur du paramètre `days` (cf. SchedulerController.ruptures). */
const MIN_HORIZON = 1
const MAX_HORIZON = 90

const EMPTY: RuptureRowsResponse = {
  rows: [],
...
export default function Shortages(props: ShortagesProps) {
  ...
  const [verdictFilter, setVerdictFilter] = useState<RuptureVerdictKey | 'all'>('all')
  ...
  // Fetch des données
  const { data, loading, error } = useTimedFetch<RuptureRowsResponse>(props.rowsHref)
  ...
  const reload = useCallback((qs: string) => {
    router.visit(`${route('ruptures.index')}?${qs}`, {
   ...
   */
  const verdictChip = (k: RuptureVerdictKey | 'all', label: string) => {
